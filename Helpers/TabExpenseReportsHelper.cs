    ...
    public class TabExpenseReportsHelper
    {
        ...
        public static DataTable getTopCategoryTotals(DateTime dtFromNew, DateTime dtToNew)
        {
            ...
            try
            {
                ...
                sql.Append(" ORDER BY sum(exp.expense_amount) DESC");
                table = DataManager.getDatatableByExecQuery(sql.ToString());
            }
            catch (Exception exception)
            {
        ...
        public static DataTable getMonthlyTotal()
        {
            ...
            try
            {
                StringBuilder sql = new StringBuilder("select month(expense_date), category_id as category, sum(expense_amount) as total FROM EXPENSES exp ");
                sql.Append(" GROUP BY month(expense_date), category_id");
                table = DataManager.getDatatableByExecQuery(sql.ToString());
            }
            catch (Exception exception)
            {
